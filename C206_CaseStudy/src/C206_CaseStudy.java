...
public class C206_CaseStudy {
	...
	public static boolean addUser(ArrayList<User> userList, User item) {
		...
		for (User user : userList) {
			boolean usernameIsEmpty = item.getUsername().isEmpty();
			boolean emailIsEmpty = item.getEmail().isEmpty();
			if (usernameIsEmpty || emailIsEmpty) {
				notEmpty = false;
			}
			boolean invalidEmailInput = !item.getEmail().contains("@");
			if (invalidEmailInput) {
				validEmail = false;
			}
			boolean duplicateUsername = item.getUsername().equalsIgnoreCase(user.getUsername());
	...
	public static boolean doDeleteUser(ArrayList<User> userList, String username, String password) {
		...
		for (int i = 0; i < userListSize; i++) {
			...
			boolean validUsernameAndPassword = userList.get(i).getUsername().equalsIgnoreCase(username)
	...
	public static String retrieveAllCurrency(ArrayList<Currency> currencyList) {
		...
		for (int i = 0; i < currencyList.size(); i++) {
			output += String.format("%-10s %-30s\n", currencyList.get(i).getCurrency_name(),
					currencyList.get(i).getCurrency_code());
	...
	public static String retrieveAllTransactions(ArrayList<Transaction> transactionList) {
		...
		for (int i = 0; i < transactionList.size(); i++) {
			...
			output += String.format("%-20s %-20s %-20s %-20s %-20s %-20s %-20s %-20s %-20s\n",
					transactionList.get(i).getUsername(), transactionList.get(i).getUserID(),
					transactionList.get(i).getPassword(), transactionList.get(i).getEmail(),
					transactionList.get(i).getRole(), transactionList.get(i).getTransactionNo(), strTD,
					transactionList.get(i).getAmount(), transactionList.get(i).getDesiredCurrency());
	...
	public static String retrieveAllAccount(ArrayList<Account> accountList) {
		...
		for (int i = 0; i < accountList.size(); i++) {
			output += String.format("%-25s %-25.2f %-25s %-25s %-25s\n", accountList.get(i).getAccount_id(),
					accountList.get(i).getAccount_balance(), accountList.get(i).getAccount_password(),
					accountList.get(i).getTransaction_history(), accountList.get(i).getDate());
	...
	public static String retrieveAllExchangeRates(ArrayList<ExchangeRate> exchangeRateList) {
		...
		for (int i = 0; i < exchangeRateList.size(); i++) {
			output += String.format("%-15s %-20s %-10s\n", exchangeRateList.get(i).getExchangeRate_code(),
					exchangeRateList.get(i).getExchangeRate_name(),
	...
	public static void viewAllExchangeRates(ArrayList<ExchangeRate> exchangeRateList) {
		...
		for (ExchangeRate exchangeRate : exchangeRateList) {
			output += String.format("%-15s %-20s %-10.2f\n", exchangeRate.getCode(), exchangeRate.getName(),
